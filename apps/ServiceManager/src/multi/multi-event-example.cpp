...
service_sample::service_sample(uint32_t _cycle, int _service,vsomeip::instance_t _instance_id, vsomeip::event_t _event_id) :
        app_(vsomeip::runtime::get()->create_application()),
        is_registered_(false),
        cycle_(_cycle),
        service_(_service),
...
bool service_sample::batteryInit() {
    std::lock_guard<std::mutex> its_lock(mutex_);
